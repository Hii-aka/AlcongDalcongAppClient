...
import { AnniversaryDate } from '../types/anniversary';
...
export function calculateAnniversaryDates(startDate: Date | string | number | null): AnniversaryDate[] {
  const validStartDate = ensureValidDate(startDate);
  if (!validStartDate) return [];

  const anniversaries: AnniversaryDate[] = [];
...
export function getNextAnniversary(startDate: Date | string | number | null): { daysUntil: number; nextAnniversary: AnniversaryDate } {
  const validStartDate = ensureValidDate(startDate);
