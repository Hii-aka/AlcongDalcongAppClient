...
import { signUpFemale } from '@/api/auth';
...
type SignupForm = {
  ...
  const {signUpFemaleMutation} = useAuth(); 

  const onSubmit = async (data: SignupForm) => {
    const {email, password, nickname} = data;
    signUpFemaleMutation.mutate({email, password, nickname});
  };
      ...
      <ScrollView className="flex-1 px-4">
        <View className="py-14 max-w-md mx-auto w-full">
          <View className="bg-white rounded-lg p-6">
            <Text className="text-2xl font-bold text-center mb-6">여성 회원가입</Text>
            <FormProvider {...signup}>  
